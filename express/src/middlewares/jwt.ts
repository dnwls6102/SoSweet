...
const isTokenPayload = (payload: any): payload is TokenPayload => { // payload의 형식을 검증하는 함수, 지금은 payload의 user_id가 문자열인지만 확인한다.
...
const renewToken = (access: string, refresh: string): string | null => {
  try {
    ...
    if(!decodedAccess || typeof decodedAccess === 'string' || !isTokenPayload(decodedAccess)) {
