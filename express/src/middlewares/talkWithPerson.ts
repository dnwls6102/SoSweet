...
async function generateGuideMessage(
  keyword: string
): Promise<{ question1: string; question2: string }> {
  try {
      ...
      messages: [
        {
          role: "system",
          content:
            "당신은 소개팅 코치입니다. 대화 상대와 함께 특정 활동을 하자고 제안하는 자연스러운 질문을 생성해주세요. 두 사람이 서로 다른 방식으로 제안할 수 있도록 서로 다른 두 개의 질문을 만들어주세요. 번호나 기호 없이 각 줄에 하나의 질문만 작성해주세요.",
        },
        {
          role: "user",
...
async function recordDialog(req: Request, res: Response): Promise<void> {
  try {
    ...
    await redisClient.rPush(
      ...
      JSON.stringify({ role: "user", content: script, name: user_id })
    );

    // 현재 방의 키워드 카운트 가져오기
    const currentKeywords = await redisClient.get(`keywords:${room_id}`);
    let keywordCounts: Record<string, number> = currentKeywords
      ? JSON.parse(currentKeywords)
      : {};
    ...
    if (keywordRef) {
      Object.entries(keywordRef).forEach(([keyword, count]) => {
        keywordCounts[keyword] =
          (keywordCounts[keyword] || 0) + (count as number);
      });
    }
    ...
    if (frequentKeywords.length > 0) {
      const randomKeyword =
        frequentKeywords[Math.floor(Math.random() * frequentKeywords.length)];
      const { question1, question2 } = await generateGuideMessage(
        randomKeyword
      );
