...
async function recordDialog(req: Request, res: Response): Promise<void> {
  try {
    const { roomID, ID, script} = req.body;
    // 아쉽게도 Redis는 문자열 기반 저장소이기 때문에, 키와 밸류 모두 문자열로 저장해야 한다.
    await redisClient.rPush(`conversations:${roomID}`, JSON.stringify({ role: "user", content: script, name: ID}));
    const record = await redisClient.lRange(`conversations:${roomID}`, 0, -1);
    console.log(record);
